...
export function FilterSidebar({ isOpen, onClose, onApply, initialFilters = {} }: FilterSidebarProps) {
  ...
  return (
    <div
      className={cn(
        "fixed inset-y-0 right-0 z-50 w-80 transform bg-white dark:bg-slate-900 shadow-xl transition-transform duration-200 ease-in-out border-l border-gray-200 dark:border-slate-700 flex flex-col backdrop-blur-md",
        ...
        <div className="p-6 space-y-6 pb-24">
          <div>
            ...
            <Select value={filters.savedFilter} onValueChange={(value) => handleRadioChange("savedFilter", value)}>
              <SelectTrigger className="w-full border-gray-300 focus:border-[#4f46e5] focus:ring-[#4f46e5]">
          ...
          {/* View By Section */}
          <div>
            <Label className="text-sm font-medium text-gray-700 mb-3 block">View By</Label>
              ...
              {viewByOptions.map((option) => (
                <label key={option.id} className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span
                    className={cn(
                      "text-sm font-medium",
                      filters.viewBy === option.id ? "text-gray-900" : "text-gray-500",
                    )}
                  >
                    {option.label}
                  </span>
